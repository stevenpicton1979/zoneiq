// Sprint 24 — SEQ Flood Overlay Ingest (Gold Coast, Moreton Bay, Sunshine Coast, Logan, Ipswich)
// All councils use ArcGIS FeatureServer/MapServer or WFS, outSR=4326
// Run: node scripts/ingest-seq-flood.js
...
// — ArcGIS FeatureServer/MapServer ingest (resultOffset pagination) —
  ...
  while (true) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        if (data.error) { console.log(`  ArcGIS error:`, data.error); features = null; break }
    ...
    if (totalFetched % 10000 < pageSize) console.log(`  Progress: fetched=${totalFetched} inserted=${totalInserted}`)
...
async function main() {
  console.log('=== SEQ Flood Overlay Ingest ===')
  let total = 0

  // Gold Coast: Designated Flood Level polygons
  total += await ingestArcGIS(
    'goldcoast',
    'https://services.arcgis.com/3vStCH7NDoBOZ5zn/arcgis/rest/services/Designated_Flood_Level_for_Residential_Buildings/FeatureServer/0/query',
    '153.2,-28.2,154.0,-27.8',
    2000,
    f => ({
      overlay_type: 'goldcoast',
      flood_category: 'Designated_Flood_Level',
      risk_level: f.properties.FLOODLVLDES?.toString() || 'unknown',
      geojson: JSON.stringify(f.geometry)
    })
  )

  // Moreton Bay: Flood Hazard Overlay
  total += await ingestArcGIS(
    'moretonbay',
  ...
  console.log(`\n=== Total inserted across all councils: ${total} ===`)
}
